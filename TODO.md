...
### 待办
- [x] 提取统一参数解析器（替换 `do_update`/`do_export` 手写解析）
- [ ] 抽离命令到模块（解析/查询/导出/外部脚本调用）
- [x] 将脚本参数白名单配置化（避免散落硬编码）
- [x] 增加 CLI 回归样例（输入命令 -> 预期命令/SQL）
...
### 待办
- [x] 将 `scripts/check_stats_api_consistency.py` 接入定时任务
- [x] 差异报告增加阈值规则（仅超阈值告警）
- [x] 扩展一致性对比维度（更多 mode/时间窗口/筛选组合）
